            ...
            header={
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <Link 
                        ...
                        className="flex items-center justify-center gap-2 bg-slate-900 text-white px-5 sm:px-6 py-2.5 rounded-full text-xs font-bold hover:bg-[#ff6b00] transition-all shadow-lg shadow-slate-200 w-full sm:w-auto"
                    >
            ...
            <div className="py-6 sm:py-12 bg-slate-50 min-h-screen">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="bg-white rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {news.map((item) => (
                                        <tr key={item.id} className="hover:bg-slate-50/30 transition-colors">
                                            ...
                                            <td className="px-8 py-6 text-sm text-slate-500 font-mono whitespace-nowrap">
                            ...
                            {news.map((item) => (
                                <div key={item.id} className="p-5 flex flex-col gap-3">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            {item.is_published ? (
                                                <span className="flex items-center gap-1 text-emerald-600 text-[10px] font-bold uppercase tracking-wider bg-emerald-50 px-2.5 py-1 rounded-full">
                                                    <Eye size={11} /> 公開中
                                                </span>
                                            ) : (
                                                <span className="flex items-center gap-1 text-slate-400 text-[10px] font-bold uppercase tracking-wider bg-slate-100 px-2.5 py-1 rounded-full">
                                                    <EyeOff size={11} /> 非公開
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="text-base font-bold text-slate-800 leading-snug break-words">
                                        {item.title}
                                    </div>

                                    <div className="flex justify-end gap-5 pt-2 border-t border-slate-50 mt-1">
                                        <Link 
                                            href={route('admin.news.edit', { news: item.id })}
                                            className="text-slate-400 hover:text-emerald-500 flex items-center gap-1 text-xs font-medium p-1"
                                        >
                                            <Edit3 size={16} /> 編集
                                        </Link>
                                        <button 
                                            onClick={() => handleDelete(item.id)}
                                            className="text-slate-400 hover:text-red-500 flex items-center gap-1 text-xs font-medium p-1"
                                        >
                                            <Trash2 size={16} /> 削除
                                        </button>
                                    </div>
                        ...
                        {news.length === 0 && (
                            <div className="p-12 sm:p-20 text-center text-slate-400 text-sm italic">記事がありません。</div>
                        )}
                    </div>
                </div>
            </div>
