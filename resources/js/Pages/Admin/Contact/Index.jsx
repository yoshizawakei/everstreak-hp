            ...
            <div className="py-6 sm:py-12 bg-slate-50 min-h-screen">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="bg-white rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-200 overflow-hidden">
                                        ...
                                        >
                                            ...
                                            <td className="px-8 py-6 text-right align-top">
                                                <div className="flex justify-end gap-3" onClick={(e) => e.stopPropagation()}>
                                                    <a href={`mailto:${contact.email}?subject=Re: お問い合わせありがとうございます（EverStreak）`} className="text-slate-300 hover:text-[#ff6b00] transition-all">
                                                        <Reply size={18} />
                                                    </a>
                        ...
                        {/* スマホ・タブレット表示用の独立カードリスト (PCでは非表示) */}
                        <div className="block sm:hidden divide-y divide-slate-100">
                            {contacts.map((contact) => (
                                <div 
                                    key={contact.id}
                                    onClick={() => setExpandedId(expandedId === contact.id ? null : contact.id)}
                                    className={`p-5 transition-colors ${expandedId === contact.id ? 'bg-slate-50' : ''} ${!contact.is_replied ? 'bg-blue-50/20' : ''}`}
                                >
                                    <div className="flex justify-between items-center mb-3">
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); toggleReplied(contact.id); }}
                                            className={`flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider ${contact.is_replied ? 'text-emerald-500' : 'text-orange-400'}`}
                                        >
                                            {contact.is_replied ? <CheckCircle size={12} /> : <Clock size={12} />}
                                            {contact.is_replied ? 'Replied' : 'Pending'}
                                        </button>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <div className="text-xs text-slate-400 break-all">{contact.email}</div>
                                    </div>

                                    <div className="mt-2">
                                        <p className={`text-sm text-slate-600 leading-relaxed break-words ${expandedId === contact.id ? '' : 'line-clamp-2'}`}>
                                            {contact.message}
                                        </p>
                                    </div>
                                    ...
                                    <div className="mt-4 flex justify-between items-center">
                                        <span className="text-[10px] text-slate-400 font-medium">
                                            {expandedId === contact.id ? (
                                                <span className="flex items-center gap-1"><ChevronUp size={12} /> 閉じる</span>
                                            ) : (
                                                <span className="flex items-center gap-1"><ChevronDown size={12} /> タップして全て読む</span>
                                            )}
                                        </span>
                                        <div className="flex gap-4" onClick={(e) => e.stopPropagation()}>
                                            <a href={`mailto:${contact.email}?subject=Re: お問い合わせありがとうございます（EverStreak）`} className="text-slate-400 hover:text-[#ff6b00] p-1">
                                                <Reply size={18} />
                                            </a>
                                            <button onClick={() => handleDelete(contact.id)} className="text-slate-400 hover:text-red-500 p-1">
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
                                    </div>
                        ...
                        {contacts.length === 0 && (
                            <div className="p-12 sm:p-20 text-center text-slate-400 text-sm italic">お問い合わせはありません。</div>
                        )}
                    </div>
                </div>
            </div>
